...
import com.shitot.converter.*;
import com.shitot.model.*;
import com.shitot.to.*;

@Repository
public class ShitotRepositoryHibernateImpl implements ShitotRepository {
	...
	public User loginUser(User user) {
		UserDAO response;
		try {
			response = em.createQuery("SELECT u FROM users u where u.name = ?1", UserDAO.class)
					.setParameter(1, user.getName()).getSingleResult();
	...
	public boolean createUser(User user) {
		List<UserDAO> users = em.createQuery("SELECT u FROM users u where u.name = ?1").setParameter(1, user.getName())
				.getResultList();
		if (users.size() == 0) {
	...
	public boolean createDoctor(Doctor doctor) {
		...
		if (doctor.getId() == 0) {
			List<DoctorDAO> response = em.createQuery("SELECT d FROM doctors d WHERE d.name=?1")
					.setParameter(1, doctor.getName()).getResultList();
			if (response.size() != 0)
				return false;
	...
	public List<Doctor> getAllDoctors() {
		List<DoctorDAO> doctors = em.createQuery("SELECT d FROM doctors d").getResultList();
	...
	public List<Patient> getAllPatient() {
		List<PatientDAO> patients = em.createQuery("SELECT p FROM patients p").getResultList();
	...
	public boolean updatePatientByDoctor(Treatment treatment) {
		...
		tret.setPaymentDate(treatment.getDatePayment());
	...
	public int getStatisticBySymptom(String nameSymptom) {
		Query q = em
				.createQuery("Select count(p) From patients p join p.problems pr join pr.symptoms s Where s.name=?1");
	...
	public List<Patient> getPatientNotMeeting() {
		Date curDate = new Date();
		Query q = em.createQuery("Select p From patients p join p.treatmant t Where t.dateMeeting<?1 and t.payment=0");
